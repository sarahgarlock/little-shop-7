...
RSpec.describe Item, type: :model do
  before :each do
    ...
    @invoice4 = Invoice.create!(customer_id: @customer2.id, status: 1, created_at: "2023-06-12 03:49:12.81835")
    ...
    @invoiceitem9 = InvoiceItem.create!(item_id: @item4.id, invoice_id: @invoice4.id, quantity: 5, unit_price: 3, status: 1)

    @transaction1 = Transaction.create!(invoice_id: @invoice1.id, cc_num: 1234567899876543, cc_exp: 23485720, result: 1)
    @transaction1 = Transaction.create!(invoice_id: @invoice4.id, cc_num: 1234567899876543, cc_exp: 23485720, result: 1)
  end
  ...
  describe "instance methods" do
    it "#best_day" do 
      ...
      expect(@item4.best_day.to_s).to eq(@invoice4.created_at.strftime("%Y-%m-%d"))
    end
  end
end